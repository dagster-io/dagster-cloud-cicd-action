...
const {getProcess, getLocations, buildDockerImages, createCommentOnCommit} = require('./utils');
  ...
  try {
    ...
    await core.group('Create code preview', async () => {
      ...
      await process(locations, async ([locationName, location]) => {
        ...
        core.info(`Successfully created repository location for code preview ${codePreviewId}`);
